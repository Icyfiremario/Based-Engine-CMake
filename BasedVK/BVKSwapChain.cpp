...
VkFormat BVKSwapChain::findDepthFormat() const
{
...
VkResult BVKSwapChain::acquireNextImage(uint32_t* imageIndex) const
{
    vkWaitForFences(device.getDevice(), 1, &inFlightFences[currentFrame], VK_TRUE, std::numeric_limits<uint64_t>::max());

    const VkResult result = vkAcquireNextImageKHR(device.getDevice(), swapChain, std::numeric_limits<uint64_t>::max(), imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, imageIndex);

    return result;
}

VkResult BVKSwapChain::submitCommandBuffers(const VkCommandBuffer* buffers, const uint32_t* imageIndex)
{
    ...
    {
        PLOGF << "Failed to submit draw command buffer.";
        throw std::runtime_error("Failed to submit draw command buffer!");
    }

    PLOGV << "Submitted draw command buffer.";
...
void BVKSwapChain::createImageViews()
{
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++)
    {
        ...
        {
            PLOGF << "Failed to create image view.";
            throw std::runtime_error("Failed to create image view!");
        }
    }

    PLOGI << "Created swap chain image views.";
}
...
void BVKSwapChain::createDepthResources()
{
    ...
    for (size_t i = 0; i < depthImages.size(); i++)
    {
        ...
        device.createImageWidthInfo(imageInfo, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImages[i], depthImageMemories[i]);
        ...
        {
            PLOGF << "Failed to create texture image depth view.";
...
void BVKSwapChain::createRenderPass()
{
    ...
    {
        PLOGF << "Failed to create render pass.";
        throw std::runtime_error("Failed to create render pass!");
    }

    PLOGI << "Created render pass.";
}
...
void BVKSwapChain::createFramebuffers()
{
    ...
    for (size_t i = 0; i < imageCount(); i++)
    {
        ...
        {
            PLOGF << "Failed to create framebuffer.";
            throw std::runtime_error("Failed to create framebuffer!");
        }
    }

    PLOGI << "Created framebuffers.";
}
...
void BVKSwapChain::createSyncObjects()
{
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
    {
        ...
        {
            PLOGF << "Failed to create synchronization objects for a frame.";
...
VkPresentModeKHR BVKSwapChain::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) const
{
...
VkExtent2D BVKSwapChain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) const
{
