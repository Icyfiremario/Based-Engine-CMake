...
VkFormat BVKSwapchain::findDepthFormat()
{
...
VkResult BVKSwapchain::acquireNextImage(uint32_t *imageIndex)
{
...
VkResult BVKSwapchain::submitCommandBuffers(const VkCommandBuffer *buffers, uint32_t *imageIndex)
{
    ...
    VkSwapchainKHR sawpChains[] = { swapChain };
    presentInfo.swapchainCount = 1;
    presentInfo.pSwapchains = sawpChains;
...
void BVKSwapchain::createSwapChain()
{
    ...
    QueueFamilyIndicies indices = device.findPhysicalQueueFamilies();
    uint32_t queueFamilyIndicies[] = { indices.graphicsFamily, indices.presentFamily };
...
void BVKSwapchain::createDepthResources()
{
    ...
    for (int i = 0; i < depthImages.size(); i++)
    {
        ...
        {
            throw std::runtime_error("Failed to create texture image depth view!");
        }
    }
}
